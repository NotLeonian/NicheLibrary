...
// 凸包と直線の共通部分を求める。
// 凸包が空、1 点、線分、面積が正の狭義凸多角形のどれであっても扱える。
// 共通部分が空集合ならば 0 個、1 点ならば 1 個、線分ならばその端点 2 個を返す。
// 以下の幾何条件は、各座標を内部計算の型へ変換した後の値について満たす。
...
// 内部計算の型が整数でない場合、符号を調べる値の絶対値が 1e-12 以下ならば 0 とみなす。
// 座標型が整数ならば整数演算で処理し、交点の座標を有理数で表して返す。
// 標準の 64 bit 以下の符号付き整数型を座標型にすると、既定で 128 bit
// 符号付き整数型を内部計算に用いる。
// 前提: 整数座標では、座標型と内部計算の型は符号付き整数型である。
// すべての中間値が内部計算の型に収まることを仮定する。
// 計算量 O(log N)。
