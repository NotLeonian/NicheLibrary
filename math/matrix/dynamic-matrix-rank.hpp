...
// 行列の階数と、行列に 1 つの外積を加えた後の階数を求める。
// さらに、内部状態を更新しながら変更後の階数を返せる。
// 現在の行列を C と R の積に階数分解し、C の左逆行列と R の右逆行列も保持する。
// 1 つの行または列の差し替えは、現在の行または列との差を用いる外積の加算に帰着させる。
// 前処理の時間計算量は O(r + rc min(r, c) + k^2(r + c))、
// 内部状態の更新や階数の計算の時間計算量は O((k + 1)(r + c)) である。
// 入力する行列は長方形であり、T は体をなすことを仮定する。
