...
// 全ての M_i が正であり、a, b, m のサイズが等しいことを仮定する。
// 入力と計算途中の値が内部の共通型に収まり、返り値が R1, R2 に収まることを仮定する。
// A_i と M_i や、異なる式の法どうしは互いに素でなくてよい。
// 解がなければ (0, 0) を返す。
// 内部の共通型が標準の 64 bit 以下の整数型であれば、剰余乗算に 128 bit 整数型を用いる。
// それ以外の整数型では加算と 2 倍による剰余乗算を用いる。
// 式の個数を N、全ての法と 2 の最大値を V とすると、時間計算量は O(N log V) である。
// ただし、内部の共通型が標準の 64 bit 以下の整数型でなければ、時間計算量は
// O(N log^2 V) である。補助空間計算量は O(1) である。
