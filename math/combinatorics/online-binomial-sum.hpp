...
// 0 <= n、0 <= l <= u、0 <= m <= max_m を仮定する。
// n > m + 1 の場合は n = m + 1 として処理する。
// n 軸と m 軸のバケット境界および上端をサンプル座標とする。
// その直積で累積和を前計算し、クエリ点に最も近いサンプル点から復元する。
// T は素数を法とする体の型である。
// std::numeric_limits<T>::is_integer が false であり、
// 法 p について max_m < p であることを仮定する。
// r = 0 では、バケットサイズを指定しない場合も既定値を求める計算を行わない。
// この場合、前計算とクエリの時間計算量および空間計算量は O(1)。
// r = -1 では交代二項和の閉形式を用いる。
// B をバケットサイズとして、r が 0, -1 でない場合の時間計算量は、
// 前計算が O(max_m^2 / B + max_m)、クエリが O(B)。
