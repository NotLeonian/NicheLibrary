...
- テンプレート引数 `Internal` で、計算途中の値に用いる整数型を指定できる。
- `Internal` を省略するか `void` を指定した場合、標準の 64 bit 以下の整数型では `NicheLibrary::Int128` を用いる。
- 計算中も $n,m,a,b$ は `T` の値として保持する。
...
- `generalized_floor_sum_degree_le_2<T, Internal>(n, m, a, b)`
  ...
  - テンプレート引数 `Internal` で、計算途中の値に用いる整数型を指定できる。
  - `Internal` を省略するか `void` を指定した場合、標準の 64 bit 以下の整数型では `NicheLibrary::Int128` を用いる。
  - 前提: `T` は `bool` 以外の整数型である。
  - 前提: `Internal` が `void` でなければ、`Internal` は `bool` 以外の整数型であり、`T` の全ての値を表せる。
  - 前提: `T` が符号付きであり、`Internal` が `void` でなければ、`Internal` も符号付きである。
  - 前提: $n\ge 0,\;m>0$ 。
  - 前提: 計算途中の値は、内部計算に用いる整数型の範囲に収まる。
  - 備考: `T` が符号付き整数型の場合、返り値が `T` の範囲に収まる必要がある。
  - 備考: `T` が符号なし整数型の場合、内部計算の結果を `T` に変換した値を返す。
  - 備考: $n,m,a,b$ および返り値の各成分が 64 bit 整数型に収まる場合は、`T` に 64 bit 整数型を指定し、`Internal` だけを広い型にすることが望ましい。
