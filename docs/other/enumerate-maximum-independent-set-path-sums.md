---
title: 列から互いに隣り合わない $k$ 個の要素を選んだときの総和の最大値または最小値の列挙
documentation_of: other/enumerate-maximum-independent-set-path-sums.hpp
---

## 概要

長さ $n$ の列 $a$ について、互いに隣り合わない $k$ 個の添字を選んだときの $a_i$ の総和の最大値または最小値を、 $0 \le k \le \lceil n / 2 \rceil$ の各整数 $k$ に対して求める。
...
- 比較ソートを用いる。
- 引数 $a$ は重みの列である。
- `maximum` の既定値は `true` である。`true` であれば最大値、`false` であれば最小値を返す。
- 返り値は、 $k$ 個選ぶときの最大値または最小値を $k$ 番目 (0-based indexing) の要素に持つ列である。
- 前提: `T()` は加法単位元を表し、`T` では加算、減算、加算代入、`<` による比較ができる。
...
- `maximum` の既定値は `true` である。`true` であれば最大値、`false` であれば最小値を返す。
- 返り値は、 $k$ 個選ぶときの最大値または最小値を $k$ 番目 (0-based indexing) の要素に持つ列である。
- 前提: `T` は `bool` でない 32 bit 以下の符号付き整数型である。
- 前提: 引数として与えられる列は非負整数列である。
- 前提: 列 $a$ の総和を $S$ とすると、 $S$ は `T` の範囲に収まり、 $2S + 1$ は `std::size_t` の範囲に収まる。
