---
title: 列の隣り合わない $k$ 個の要素を選んだ総和の最大値の列挙
documentation_of: other/enumerate-maximum-independent-set-path-sums.hpp
---

## 概要

長さ $n$ の列 $a$ について、隣り合わない $k$ 個の添字を選んだときの $a_i$ の総和の最大値を、全ての $k$ に対して列挙する。

- パス上の最大重み独立集合について、独立集合の大きさごとの最大重みを求めるものとみなせる。
- 返り値の長さは $\lceil n / 2 \rceil + 1$ である。
- 返り値の $k$ 番目 (0-based indexing) の要素は、 $k$ 個選んだときの総和の最大値である。

## 使い方

`enumerate_maximum_independent_set_path_sums(a)`

- 比較ソートを用いる。
- 引数 $a$ は重みの列である。
- 返り値は、$k$ 個選ぶときの最大値を $k$ 番目 (0-based indexing) の要素に持つ列である。
- 前提: `T` は加算、減算、比較が正しく定義されている。
- 前提: アルゴリズム中に現れる加減算結果、および返り値の各要素が `T` の範囲に収まる。
- 前提: `std::is_unsigned_v<T>` が `false` である。

`enumerate_maximum_independent_set_path_sums_bucket_sort(a)`
