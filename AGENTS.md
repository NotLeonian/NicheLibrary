...
- ドキュメントの `## 計算量` に記載されている、big-O 記法などのランダウの記法による計算量オーダーは、ランダウの記法の数学的な定義に従った厳密な値であり、ランダウの記法を用いることが自然なものである。
...
- ドキュメントの `## 計算量` で、平均計算量や期待計算量などの最悪計算量と償却計算量以外の計算量を述べる場合は、その旨を説明している。
